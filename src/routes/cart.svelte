			...
			<ul class="cart_list">
				{#each $cartCollection as item, idx (item.elem.name)}
					<li class="cart_item" animate:flip={{ duration: 200 }}>
						...
						<a href="/{item.categoryId}/goodItems/{item.elem.id}" class="item_title"
							>{item.elem.name}</a
						>
						...
						<div class="item_counter">
							<button
								on:click={() =>
									item.cartCounter <= 1 ? (item.cartCounter = 1) : --item.cartCounter}
				...
				<div class="controls_wrap">
					<div class="cart_controls">
						...
						{#if promoObj.promocode}
							<form
								class="promocode_form"
								on:submit|preventDefault={approveCode(promoObj.promoValue)}
							>
								<div class="form_container">
									<input
										type="text"
										class="promo_inp"
										placeholder="для теста 123"
										bind:value={promoObj.promoValue}
									/>
